...
```
┌── 阶段①：写代码 + 准备（🧑A负责，此时没跑任何模型）──┐
│                                                       │
│  human_concepts.csv  ──►  extract.py / analysis.py     │
│  (1105 items)            定义 prompt/池化/产出格式      │
│                                                       │
└──────────────────────────┬────────────────────────────┘
                           │  🧑A 写完后，三台机器 `git pull` 拿到代码
                           ▼
┌── 阶段②：唯一一次 Embedding 提取（三人并行）─────────────┐
│                                                         │
│  🧑A 抽本地模型①（RTX 4060, ~12个, ~30min）              │
│  🧑B 抽本地模型②（RTX 4060, ~8个,  ~30min）              │
│  🧑C 抽学校大模型（USTC 107, ~8个,  ~60min）             │
│                                                         │
│  三人都跑同一份 extract.py，产出统一格式：                  │
│  models_embeddings/{model_name}.npy (shape: 1105, dim)   │
│                                                         │
└──────────────────────────┬──────────────────────────────┘
                           │  🧑A 收齐所有人产的 .npy
                           ▼
